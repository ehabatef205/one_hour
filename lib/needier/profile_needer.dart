...
import 'edit_profile_needer.dart';
import 'package:http/http.dart' as http;
import 'home_page_needer.dart';
import 'dart:ui' as ui;

class ProfilePageNeeder extends StatefulWidget {
  final bool isArabic;
  final String user_id;

  ProfilePageNeeder({this.user_id, this.isArabic});

  @override
  _ProfilePageNeederState createState() => _ProfilePageNeederState();
}

class _ProfilePageNeederState extends State<ProfilePageNeeder> {
  List language;
  var data;
                      ...
                      TextButton(
                        ...
                        onPressed: () {
                          Navigator.push(
                              context,
                              MaterialPageRoute(
                                  builder: (context) => SettingNeederNeeder(
                                    isArabic: widget.isArabic,
                                        list: data,
                                      )));
                        },
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return Column(
                                    children: [
                                                ...
                                                Container(
                                                  ...
                                                  decoration: BoxDecoration(
                                                    ...
                                                    image: DecorationImage(
                                                      fit: BoxFit.cover,
                                                      image: NetworkImage(
                                                          "http://$local/test/uploads/needer/${data['image']}"),
                                                    ),
                                                  ),
                                                ),
                                              ],
                                      ...
                                      newMethod(
                                          "${language[13]}:", data["ssn"]),
                                      newMethod(
                                          "${language[28]}:", data["email"]),
                                      newMethod("${language[18]}:",
                                          data["phone"]),
